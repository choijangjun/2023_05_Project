			...
			<section class="myPage-center-section">
				<div class="myPage-center-div">
					<div class ="myPage-head-div">
						<h2 class ="myPage-head-h2">이벤트 수정</h2>
					</div>
						<div class="writeEvent-box">
							...
							<div>
								<input class="writeEvent-input" type="text" name="name" value="${eventArticle.name }" placeholder="상호명을 입력해주세요."/>
							</div>
						</div>
						<div class="writeEvent-box">
							<div class="writeEvnet-input-head">
							...
							<div>
								<input class="writeEvent-input" type="text" name="title" value="${eventArticle.title }" placeholder="이벤트의 제목을 입력해주세요."/>
							</div>
						</div>
						<div class="writeEvent-box">
							<div class="writeEvnet-input-head">
							...
							<div>
								<input class="writeEvent-input" type="text" name="site" value="${eventArticle.site }" placeholder="이벤트가 진행중인 주소를 입력해주세요."/>
							</div>
						</div>
						<div class="writeEvent-box">
							<div class="writeEvnet-input-head">
							...
							<form id="uploadForm">
								<div id="writeEvent-imageUpload" class="writeEvent-imageUpload">
									<input id="writeEvent-image" type="hidden" name="image" value="${eventArticle.image }"/>
									...
									<c:if test="${eventArticle.image  != ''}">
										<img class="writeEvent-imageUpload-image" src="${eventArticle.image }"/>
									</c:if>
									<c:if test="${eventArticle.image  == ''}">
										<img class="writeEvent-imageUpload-image" src="/usr/eventArticle/file/${eventArticle.id}"/>
									</c:if>
								</div>
							</form>
							<div>
								...
								<div id="writeEvent-imageURL" class="writeEvent-imageURL">
									<div style="margin: 20px; width:400px; background-color: #fff;">
										...
										<div>
											<input id="writeEvent-imageURL-input" class="writeEvent-imageURL-input"  type="text" value="${eventArticle.image }" placeholder="이미지 URL주소를 입력하세요."/>
										</div>
										<div >
						...
						<div class="writeEvent-box">
							...
							<div class="Date-div">
								<div class="writeEvent-Date-div writeEvent-startate">
									<input class="writeEvent-input-Date"  type="text" name="startDate" value="${eventArticle.startDate }" placeholder="시작 날짜을 입력하세요."/>
									<i class="calendar-icon fa-solid fa-calendar-days"></i>
								</div>
								<span>~</span>
								<div class="writeEvent-Date-div writeEvent-endDate">
									<input class="writeEvent-input-Date" type="text" name="endDate" value="${eventArticle.endDate }" placeholder="종료 날짜를 입력하세요."/>
									<i class="calendar-icon fa-solid fa-calendar-days"></i>
								</div>
								<div class="writeEvent-Date-div writeEvent-winningDate" >
									<input class="writeEvent-input-Date"  type="text" name="winningDate" value="${eventArticle.winningDate }" placeholder=" 당첨일을 입력하세요."/>
									<i class="calendar-icon fa-solid fa-calendar-days"></i>
								</div>
							</div>
						...
						<div class="writeEvent-box">
							...
							<div>
								<input class="writeEvent-input" type="text" name="goods"  value="${eventArticle.goods }" placeholder="상품을 입력하세요."/>
							</div>
							<div class="writeEvnet-input-head">
								<h3>참여 방법</h3>
							</div>
							...
							<div>
								<input class="writeEvent-input" type="text" name="needs" value="${eventArticle.needs }"  placeholder="조건을 입력하세요."/>
							</div>
							<div class="writeEvnet-input-head">
								<h3>가격</h3>
							</div>
							<div>
								<input class="writeEvent-input" type="number" name="price" value="${eventArticle.price }"  placeholder="가격을 입력하세요."/>
							</div>
						</div>
						<div class="writeEvent-btn-div">
								<button id="writeEvent-btn" class="writeEvent-btn btn-hover" >수정완료</button>
						</div>
...
<script>
	$(function(){
		<c:forEach items="${eventArticle.kategorie}" var="kategorie" varStatus="status">
			if('${kategorie}' == '영화') {
...
$(function(){
	$(document).on("click", "button[id='writeEvent-btn']", function() {
		...
		$.ajax({
			type: "get"
			, url: "/usr/eventArticle/doModifyEventArticle?name=" + inputName +
					"&id=" + inputId +
					"&title=" + inputTitle +
					"&site=" + inputSite +
					"&image=" + inputImage +
			...
			, success: function(data) {
				alert("[" + inputTitle + "] \n 이벤트 수정이 완료되었습니다.");
				location.replace("/usr/myPage/myWriteEventArticle");
			}
		});
	});
});
